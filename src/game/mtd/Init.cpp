...
extern "C" int Init(int argc, char ** argv) {
	...
	*/
	// create animated bow
	Entity *animatedBow = engine->AddEntity(engine->GetNewEntityOfType("DynamicEntity"), engine->GetAvailableEntityName("Bow"), engine->GetResourceManager()->GetCylinder(0.3, 1), btTransform(btQuaternion(btVector3(1,1,1),0), btVector3(15,3,15)), 3.0f);
	...
	// create player
	Entity *player = engine->AddEntity(engine->GetNewEntityOfType("Player"), "Player", engine->GetResourceManager()->GetCylinder(0.3f, 1.75f), btTransform(btQuaternion(btVector3(1,1,1),0), btVector3(-34,25,14)), 75.0);
	...
	for(float x = -100; x<=100.1; x+=24) {
		for(float z = -100; z<=100.1; z+=24) {
			Entity *map = engine->AddEntity(engine->GetNewEntityOfType("StaticEntity"), "TestMap" + std::to_string(x) + "_" + std::to_string(z), 
	...
	Entity *map = engine->AddEntity(engine->GetNewEntityOfType("StaticEntity"), "TestMap", mapShape, btTransform(btQuaternion(btVector3(1,1,1),0), btVector3(0,-100,0)), 100000000.0f);
	map->SetModel(engine->GetResourceManager()->GetModel("Models/TechDemoMap.x"));
	map->SetScale(btVector3(3, 3, 3));
	for(float x = -10; x<=10.1; x+=10) {
		for(float y = 30; y<=75.1; y+=3.5f) {
			for(float z = -10; z<=10.1; z+=10) {
				Entity *box = engine->AddEntity(engine->GetNewEntityOfType("DynamicEntity"), std::to_string(x)+"_"+std::to_string(y)+"_"+std::to_string(z)+"box", engine->GetResourceManager()->GetBox(btVector3(1,1,1)), btTransform(btQuaternion(btVector3(1,1,1),0), btVector3(x,y,z)), 75.0f);
