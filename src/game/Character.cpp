...
#include <engine/CollisionObjectManager.h>
...
void Character::Tick(const float deltaTime) {
	...
	static TimeCounter fpsCounter = []()->TimeCounter{TimeCounter cnt;
			cnt.SetTimeSpan(2.0f); fpsCounter.SubscribeStart(); return cnt;}();
	fpsCounter.SubscribeEnd();
	sing::gui << Rectanglef(0.05,0.02,0.6,0.6) << "Character position: " <<
...
void Character::ApplyDamage(float damage, btVector3 point, btVector3 normal) {
...
Character::Character(const JSON& json) :
	...
	btCollisionObject *collisionObject =
			CollisionObjectManager::CreateRigidBody(collisionShape,
					currentTransform, 15.0f, btVector3(0,0,0));
	...
	SetBody(collisionObject, collisionShape, CollisionDefaultGroupCharacter,
			CollisionDefaultMaskCharacter);
